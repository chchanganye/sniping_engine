...
async function loadGoodsByCategory(
  frontCategoryId: number,
  groupHints?: Array<{ id: number; name?: string }> | undefined,
) {
  const token = currentAccount.value?.token
  if (!token) return
  try {
    await goodsStore.loadGoodsByCategory(token, frontCategoryId, 1, 500, groupHints)
  } catch (e) {
...
async function onTreeNodeClick(node: ShopCategoryNode) {
  ...
  const bindChildren = (node as any)?.extra?._bindChildren as Array<{ id: number; name?: string }> | undefined
  const groupHints = Array.isArray(bindChildren) && bindChildren.length > 0 ? bindChildren : undefined
  await loadGoodsByCategory(node.id, groupHints)

  // 兼容“左侧节点是分组容器，真实商品在父分类里通过标签(categoryId)区分”的场景：
  // - 先按 node.id 请求
  // - 若返回空，再按 node.pid 请求，并自动选中该节点的第一个绑定标签（若有）
  const hintIds = new Set((groupHints ?? []).map((c) => Number(c.id)).filter((v) => Number.isFinite(v) && v > 0))
  const apiMatchesHints = hintIds.size > 0 && apiSkuGroups.value.some((g) => hintIds.has(g.categoryId))
  if ((!apiSkuGroups.value.length || !apiMatchesHints) && Number(node.pid) > 0 && hintIds.size > 0) {
    const parentId = Number(node.pid)
    await loadGoodsByCategory(parentId, groupHints)
    const parentMatchesHints = apiSkuGroups.value.some((g) => hintIds.has(g.categoryId))
    if (parentMatchesHints && groupHints?.length === 1) {
      goodsStore.setSelectedGroupId(Number(groupHints[0]!.id))
    } else if (!parentMatchesHints) {
      goodsStore.setSelectedGroupId(undefined)
      await loadGoodsByCategory(node.id, groupHints)
    }
  }
